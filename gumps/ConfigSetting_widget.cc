...
ConfigSetting_widget::ConfigSetting_widget(
	...
	if (default_choice == -1)
	{
		...
		setting.choices.push_back(ConfigSetting_widget::Definition::Choice{
				setting.default_value, setting.default_value, setting.default_value});
	}
	else
	{
	}

	// Add in the empty choice
	...
	for (int i = -1; i < setting.additional; i++)
	{
		...
		children[i + 1] = std::make_unique<
					CallbackButtonBase<ConfigSetting_widget, StringList_widget>>(
					this, &ConfigSetting_widget::onselectionmb, sv_choices,
		...
		{
			children[i + 1] = std::make_unique<
					SelfManagedCallbackToggleTextButton<
...
void ConfigSetting_widget::paint() {
	...
	// If a label is longer than 64 chars, someone is doing something wrong and this code will truncate the string as needed
	char         m[64];
	const size_t len = setting.label.copy(
			m, std::size(m) - (Count_digits10(setting.additional) + 3), 0);

	for (int i = -1;
		 i < setting.additional && (i < 0 || size_t(i) < children.size()); i++) {
...
void ConfigSetting_widget::save_to_config(bool write_file) {
	for (int i = -1;
		...
		if (sel != missingchoice) {
			...
			config->set(
								  config_key,
					setting.choices[sel].value, false);
		}
	}

	if(write_file) config->write_back();
}
...
std::string ConfigSetting_widget::Validate() {
	...
	{
		return "At least One value for "+setting.label+" setting must be set. ";
...
int ConfigSetting_widget::Definition::find_choice(std::string_view value) const{
	for (size_t i = 0; i < choices.size();i++)
	{
		auto& choice = choices[i];
		if ((!Pentagram::strncasecmp(
					choice.value.c_str(), value.data(), value.size()) && value.size() == choice.value.size())
			|| ( !choice.alternative.empty() && !Pentagram::strncasecmp(
	...
	std::sort(
			...
			[](const Choice& left, const Choice& right) {
				return Pentagram::strcasecmp(
						left.label.c_str(), right.label.c_str()) <0;
			});
}
