...
void Lab_schedule::init(
	...
	Game_object *book_obj = NULL, *chair_obj = NULL;
	...
	for (int i = 0; (!book_obj || !chair_obj) && i < cnt; i++) {
		...
		Rectangle foot = table->get_footprint();
		// Book on table?
		if (!book_obj && (book_obj = table->find_closest(642, 4)) != 0) {
...
static int Count_waiter_objects(Actor *npc)
{
	int cnt = 0;
	for (int i = 0; i < waiter_nshapes; ++i)
		cnt += npc->count_objects(waiter_shapes[i]);
	return cnt;
}
                              ...
                              bool cooking = false)
{
	...
	if (!cooking || !cnt) {
		for (int i = 0; i < waiter_nshapes; ++i) {
			cnt += npc->get_objects(items, waiter_shapes[i],
			                               c_any_qual, c_any_framenum);
		}
	}
	return cnt;
	...
	case prep_food: {
		...
		if ((!cooking || rand()%3) &&
			...
			if (rand()%3) {
				...
				to_drop = items_in_hand ? items[rand()%items_in_hand] : 0;
			}
			cooking = true;
			if (to_drop)
...
void Sew_schedule::now_what(
	...
	case to_work_table: {
		...
		Shape_info &info = work_table_obj->get_info();
	...
	case display_clothes: {
		...
		Shape_info &info = wares_table_obj->get_info();
...
Bake_schedule::Bake_schedule(Actor *n) : Schedule(n),
	...
	dough(), dough_in_oven(), state(find_leftovers)
{ }
...
void Bake_schedule::now_what() {
	...
	case to_table: {    // Walk over to worktable and create flour
		...
		Shape_info &info = worktable_obj->get_info();
		Tile_coord cpos(foot.x + rand() % foot.w, foot.y + rand() % foot.h,
	...
	case display_wares: {
		...
		Actor_action *pact = Path_walking_actor_action::create_path(
		                         npcpos, spot, COST);
		Shape_info &info = displaytable_obj->get_info();
		spot_on_table.tz += info.get_3d_height();
	...
	case put_in_oven: { // Walk to oven and put dough on in.
		...
		Shape_info &info = oven_obj->get_info();
...
void Forge_schedule::now_what(
	...
	case put_sword_on_firepit: {
		...
		Shape_info &info = firepit_obj->get_info();
	...
	case sword_on_anvil: {
		...
		Shape_info &info = anvil_obj->get_info();
