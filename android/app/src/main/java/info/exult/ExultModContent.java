...
class ExultModContent extends ExultContent {
	...
	public ExultModContent(String gameToMod, String modName, Context context) {
		...
		m_installPath
				= context.getFilesDir().toPath().resolve(gameToMod).resolve(
						"mods");
	}
	...
	protected boolean identify(
		...
		if (!fullPath.getFileName().toString().toLowerCase().endsWith(
					m_modName + ".cfg")) {
			Log.d("ExultModContent", "not " + m_modName + ".cfg");
			return false;
		}

		// A valid mod configuration should contain XML with a root tag of
		// "mod_info"
		...
		// If we got here, we have the mod we're looking for.  Note the
		...
		Log.d("ExultModContent", "done - rootInArchive=" + m_modRootInArchive
