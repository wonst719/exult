...
/**
 * XarSource which reads from an arbitrary InputStream. Note that XarSource is
 * supposed to support random access, but this implementation assumes getRange
 * will always move forward. The implication is that a XarEntry must be read in
 * order while traversing the archive, and that a given it can only be read
 * once. Any attempt to read a prior XarEntry or reread the current XarEntry
 * will result in an exception due to InputStreams inability to move backwards.
 * Also note that getSize() will return the initial value of
 * InputStream.available(), but this may be inaccurate for non-file streams.
 */
class InputStreamXarSource extends XarSource {
	...
	InputStreamXarSource(InputStream inputStream) {
		...
		try {
			...
			Log.d("InputStreamXarSource",
				  "exception getting size: " + e.toString());
		}
		m_size   = size;
		m_offset = 0;
	}
	...
	@Override
	public BufferedSource getRange(long offset, long length)
			throws IOException {
