...
void Laurianna object#(0x494) () {
	...
	if (queststate < PLAYER_KILLED_MAGE) {
		return;
	}
	var bark;
	if (event == CLEAR_FLAGS) {
		...
		*/
		...
		if (queststate >= POTION_WAS_USED) {
			bark = "@Hello again, Laurianna.@";
		} else {
			bark = "@Might we speak?@";
		}
		...
		if (queststate >= POTION_WAS_USED) {
			...
			bark = "@Thou dost have no potions, right?@";
		}
